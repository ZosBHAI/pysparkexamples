...
class MergeSchemaHelper:
    ...
    def get_modified_partitions(self, dir_path, ts_filter):
        ...
        if lst_dir != []:
            ...
            return sorted([(dir, round(os.stat(dir_path + "/" + dir).st_mtime * 1000)) for dir in lst_dir if
                           round(os.stat(dir_path + "/" + dir).st_mtime * 1000) > ts_filter])
        else:
            return []
...
class MergeSchema:
    ...
    def identify_schema_changes(self, path, file_format, partition_list=[]):
        ...
        for dir in sorted(
                [d for d in os.listdir(path) if d.find("=") != -1]) if partition_list == [] else partition_list:
    ...
    def merge_schemas(self, dir_path, file_format, mode, ctrl_file):
        ...
        if merge_schema_helper.get_modified_partitions(dir_path, last_modified_ts_ctrl) != []:
            lst_dir = [dir for dir, last_mod_ts in
                       merge_schema_helper.get_modified_partitions(dir_path, last_modified_ts_ctrl)]
